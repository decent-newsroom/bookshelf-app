...
object BookRatingEventParser {
    fun parse(event: NostrEvent): BookRatingParseResult {
        ...
        val coordinate = parseBookTarget(target)
            ?: return BookRatingParseResult.Rejected("The d tag is not a books kind-30040 target.")
        val entityTypes = event.tags.valuesFor("m")
        if (entityTypes.size > 1 || entityTypes.singleOrNull()?.let { it != BOOKS_NAMESPACE } == true) {
            return BookRatingParseResult.Rejected("The optional m tag conflicts with the books target.")
        }
    ...
    fun parseBookTarget(target: String): String? {
        val namespaceEnd = target.indexOf(':')
        if (namespaceEnd <= 0 || target.substring(0, namespaceEnd) != BOOKS_NAMESPACE) return null
    ...
    private const val BOOKS_NAMESPACE = "books"
