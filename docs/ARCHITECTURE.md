...
Opening community ratings reads cached events first and shows the resulting list without waiting for relays. With validated internet, the repository refreshes relay ratings in the background and updates the visible list only when a newly discovered event changes the selected rating set. A relay failure leaves the cached list visible. `ValidatedInternetConnectivity` is the application-scoped source for validated internet state; while it is offline, rating reads and suggestions use only `BookRatingCache` and start no rating relay request.
