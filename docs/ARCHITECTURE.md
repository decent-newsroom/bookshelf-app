...
Settings groups appearance, account actions, relays, and cache controls into collapsible sections. The Relays section shows the fixed search APIs, editable chapter-publication relays, and read-only NIP-65 read/write relay lists. An optional editable local `ws://` or `wss://` relay (for example Citrine on the same phone) is stored in app-private preferences and is added to the directory client's bootstrap read/write relays; it is not a chapter source. Empty disables it.
