...
- Reader highlights are selected from that same displayed `AnnotatedString`, stored with durable UTF-16 offsets plus contextual anchors, and rendered again when the chapter reopens. Publishing keeps the private record and routes its signed NIP-84 event through the durable highlight outbox; signer requests are launched only from the app UI callback.
