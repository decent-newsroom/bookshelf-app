...
Contextual onboarding is app-private. `OnboardingTipStore` records stable tip identifiers only after a tooltip closes, whether the person discovers the described control or skips it. Each new UI affordance receives its own identifier, so newly added tips remain eligible for people who have completed earlier ones.
